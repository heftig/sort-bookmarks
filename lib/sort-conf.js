...
(async function() {
  let conf = (await browser.storage.sync.get(["sortConf"])).sortConf;

  if (!conf) {
    // Migrate 0.2 settings
    let keys = ["by", "folders", "reversed"], storKey = (k) => `popupForm-${k}`;
    let values = await browser.storage.sync.get(keys.map(storKey));

    conf = {};
    for (let k of keys) conf[k] = values[storKey(k)];
